...
function GoToTarget(x, y, z, vehicle, driver, vehhash, target)
    ...
    ShowAdvancedNotification(companyIcon, companyName, "Mechanic", "Help sent to the point where you had your accident. ~y~" .. companyName)  ---- Yardım istediği zaman giden mesaj ---
    enroute = true
    while enroute do
        Citizen.Wait(500)
...
function RepairVehicle(target, vehicle, driver)
    ...
    Citizen.Wait(500)
	ShowAdvancedNotification(mechPedPick.icon, mechPedPick.name, "Mechanic" , mechPedPick.lines[math.random(#mechPedPick.lines)])   ----- Hizmet adı -----
	...
	ShowAdvancedNotification(mechPedPick.icon, mechPedPick.name, "Mechanic" , "~b~Have a good day!")
end
...
function GetTargetVehicle(player, dir)
    ...
    if DoesEntityExist(dmgVeh) then
        ...
        ShowNotification("You must be near the vehicle to request.")       --- Aracın yanında değilse giden mesaj ----
    end
end
