...
AddEventHandler('TireEntendu', function(gx, gy, gz)
     if PlayerData.job ~= nil and PlayerData.job.name == 'police' then
          ...
          ESX.ShowAdvancedNotification(
               'LSPD INFORMATION', 
               'CENTRAL LSPD', 
               '~b~A citizen heard gunshots, hurry up!~s~ \n\n~g~[Y]~s~ : To answer the call~s~\n\n~r~[X]~s~ : To refuse.~s~', 'CHAR_CHAT_CALL', 8)
          coordsX = gx
          coordsY = gy
          coordsZ = gz
          Citizen.Wait(1000)
...
AddEventHandler('TireEntenduBlips', function(gx, gy, gz)
     if PlayerData.job ~= nil and PlayerData.job.name == 'police' then
          if AlertePrise then
               ...
               AddTextComponentString('Gunshot')
...
AddEventHandler('PriseAppel', function(name)
     if PlayerData.job ~= nil and PlayerData.job.name == 'police' then
          ...
          PlaySoundFrontend(-1, "On_Call_Player_Join", "DLC_HEISTS_GENERAL_FRONTEND_SOUNDS", 0)
          PlaySoundFrontend(-1, "On_Call_Player_Join", "DLC_HEISTS_GENERAL_FRONTEND_SOUNDS", 0)
          ESX.ShowNotification('~w~The agent ~g~'..name..'~w~ answered the call')
     end
end)
...
Citizen.CreateThread(function()
     while true do
          ...
          elseif IsControlJustPressed(1, 73) and alerteEnCours then
               AlertePrise = false
               alerteEnCours = false
               ESX.ShowNotification('~w~You declined the call')
          end
     end
end)