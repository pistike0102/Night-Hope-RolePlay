...
if Config.Commands then
    ...
    RegisterNetEvent('esx-qalle-needs:pipi')
      AddEventHandler('esx-qalle-needs:pipi', function()
        if Config.StatusBars then
            TriggerEvent('esx_status:getStatus', 'pee', function(status)
                if status.val < 200000 then
                    ...
                    ESX.ShowNotification('Você não precisa fazer xixi')
                end
            end)
        else
            local hashSkin = GetHashKey("mp_m_freemode_01")
    ...
    RegisterNetEvent('esx-qalle-needs:shit')
      AddEventHandler('esx-qalle-needs:shit', function()
        if Config.StatusBars then
            TriggerEvent('esx_status:getStatus', 'shit', function(status)
                if status.val < 200000 then
                    ...
                    ESX.ShowNotification('Você não precisa fazer cocô')
                end
            end)
        else
            TriggerServerEvent('esx-qalle-needs:sync', GetPlayerServerId(PlayerId()), 'poop')
